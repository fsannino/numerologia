...
- **Numerologia Cabalística (latina) — planejada**: escola de variantes explícitas obrigatórias (tabela `sequential-1-9` | `chaldean-like-1-8` | `hebrew-gematria` × redução `decimal` | `modular-22`), sem default silencioso — sem escolha, mostram-se todas as leituras. Delega a aritmética aos engines já existentes (R4: não reescreve tabelas), o que torna a detecção de coincidência ("≡ Pitagórico") *provadamente* verdadeira. Opera **só sobre o nome** (a data não entra — característica da escola, não omissão). Redução arcano mod-22 é a única aritmética nova.
- **231 Portões (Sefer Yetzirah) — planejada**: explorador **estrutural**, não interpretativo. As 231 combinações de pares derivadas das 22 letras hebraicas (nunca hardcoded), reuso da transliteração da Gematria, saída = estrutura (grafo), escalar = nº de portões ativados. Modos de aplicação ao nome rotulados como construção contemporânea (sem fonte canônica). Zero texto interpretativo. Testes de propriedade, não de fixture.
